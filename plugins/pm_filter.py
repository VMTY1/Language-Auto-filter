...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
          ab = await message.reply_photo(photo=imdb.get('poster'), caption=cap[:1024], reply_markup=InlineKeyboardMarkup(btn))
          if settings['auto_deletee']:
...
async def advantage_spell_chok(msg, message):
    ...
    InlineKeyboardButton("🔍 Click To Check IMDB details", url=f"https://www.imdb.com/find?q={reply}")
    ]]
    )    
